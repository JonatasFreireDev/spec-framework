...
The Technical Landscape catalog is an index: each category maps a stable entity ID to a relative entity YAML file, never to an embedded entity mapping. Referenced records carry their own schema version, matching identity, category-compatible type, and lifecycle status so a newly materialized baseline is mechanically verifiable.

Engineering Standards and Operations catalogs follow the same strict indexing rule. Profiles, standards, exceptions, environments, and operational deployments live in referenced records with matching identities and required lifecycle fields; runbooks use referenced Markdown files. Unknown Engineering System areas, malformed topology or fitness-function catalogs, embedded records, invalid references, and structurally invalid referenced records block validation.
