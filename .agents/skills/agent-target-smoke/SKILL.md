...
description: Smoke-test Spec Framework initialization and upgrade output for all supported agent targets and their Guide-first dispatchers.
---
...
1. Inspect the isolated fixture, initialize it for each requested supported target with explicit `--code-roots` or confirmed `--no-code-roots`, and reject an unreviewed CLI fallback.
