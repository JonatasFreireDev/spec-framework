...
description: Scaffold a new agent skill in framework/skills/ following the repository's skill contract. Use when adding a specialist or orchestrator skill to the framework product, renaming an existing one, or bringing a skill's structure up to the canonical format.
---
...
Skills in `framework/skills/` are embedded in the framework CLI and shipped through the selected agent discovery trees. A new skill that deviates from the canonical structure, hardcodes lab paths, or goes unregistered degrades every adopter repository at once.
...
---
name: <skill-name>
description: "<Name> Skill. Use when an agent needs to <responsibility, lowercase continuation> in the Spec Framework workflow, including creating, updating, auditing, explaining, routing, or handing off related product artifacts."
---
...
Run the `verify` skill and inspect the description, path contract, README registration, and every generated agent target.
