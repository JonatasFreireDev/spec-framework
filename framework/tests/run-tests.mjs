...
const validatorScript = path.join(repoRoot, "framework", "validators", "framework-validator.mjs");
const moveScript = path.join(repoRoot, "framework", "tools", "move-artifact.mjs");
...
function scaffoldBase(root) {
  ...
  write(root, "framework/template/minimal-template.md", "# Minimal Template\n\n## Snapshot\n\n| Field | Value |\n| --- | --- |\n| Purpose | Test fixture. |\n");
  mkdir(path.join(root, "audits", "readiness"));
  copy(root, moveScript, "framework/tools/move-artifact.mjs");
...
test("validator checks real Markdown links inside templates", () => {
  withFixture("template-link", (root) => {
    write(root, "framework/template/template-with-link.md", "# Template\n\n## Snapshot\n\n[Broken](missing-template-target.md)\n");

    const result = runValidator(root);

    assert.notEqual(result.status, 0, output(result));
    assert.match(output(result), /framework\/template\/template-with-link\.md/);
...
test("npm package includes framework assets required by bootstrap", () => {
  ...
  for (const required of [
    ...
    "framework/skills/code-runner/SKILL.md",
    "framework/template/specification-template.md",
    "framework/validators/framework-validator.mjs",
    "framework/tools/move-artifact.mjs",
    "FRAMEWORK.md",
    "AGENTS.md",
  ]) {
...
test("project skills resolve framework and product paths through explicit roots", () => {
  const skillFiles = walk(path.join(repoRoot, "framework", "skills"))
