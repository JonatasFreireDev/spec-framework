...
function knownSkillNames() {
  const skillDirs = [
    ...
    path.join(frameworkRoot, "framework", "skills"),
...
function buildArtifactsRegistry() {
  ...
  return {
    ...
    generator: "framework/validators/framework-validator.mjs",
    artifacts,
  };
}
    ...
    for (const node of graph.nodes) {
      ...
      if (node.path) {
        ...
        if (!taskFile.startsWith(graphDir)) {
          ...
          addResult("error", "execution-graph", file, `Node ${node.id} task file does not exist: ${node.path}`, "Create the task file from framework/template/task-template.md.");
        } else {
...
function validateContexts() {
  for (const file of findContextFiles()) {
    ...
    if (!meta.slug) {
      ...
      addResult("error", "identity", file, `Context slug ${meta.slug} does not match folder ${path.basename(path.dirname(file))}.`, "Keep slug equal to the immutable folder name or move the folder with framework/tools/move-artifact.mjs.");
    }
...
function validateIdentityPolicy() {
  ...
  const moveToolCandidates = [
    ...
    path.join(frameworkRoot, "framework", "tools", "move-artifact.mjs"),
  ...
  if (!fs.existsSync(moveTool)) {
    addResult("error", "identity", moveTool, "Move tooling is missing.", "Create framework/tools/move-artifact.mjs or .spec-framework/tools/move-artifact.mjs.");
  }
}
...
function validateMermaidAndTemplates(files) {
  ...
  const templateDir = fs.existsSync(path.join(frameworkRoot, "framework", "template"))
    ? path.join(frameworkRoot, "framework", "template")
    : root === cwd && fs.existsSync(path.join(root, "knowledge", "templates"))
...
function isTemplateGeneratedArtifactLink(file, target) {
  const fileRel = rel(file);
  if (!fileRel.startsWith("framework/template/")) return false;
...
| --- | --- |
| Date | ${now} |
| Validator | \`framework/validators/framework-validator.mjs\` |
| Verdict | ${verdict()} |
| Errors | ${counts.error} |
