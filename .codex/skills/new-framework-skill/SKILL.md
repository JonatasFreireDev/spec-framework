...
Skills in `framework/skills/` are embedded in the Go CLI and ship under `.spec-framework/skills/` plus the selected Codex, Cursor, and Claude Code discovery trees. A new skill that deviates from the canonical structure, hardcodes lab paths, or goes unregistered degrades every adopter repository at once.
