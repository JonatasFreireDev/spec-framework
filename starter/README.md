...
9. Use `engineering-orchestrator` to establish the shared technical baseline, `spec-framework engineering-system inspect` to inspect its aggregate, `spec-framework dashboard --work WORK-001` for the consolidated flow, and `spec-framework decisions migrate` to preview legacy decision metadata upgrades.
